...
class Encoder(nn.Module):
    def __init__(self,d_model,d_k,d_v,n_heads,d_ff):
        ...
        self.pos_emb = nn.Embedding.from_pretrained(get_sinusoid_encoding_table(6, d_model),freeze=True)
        self.layers = nn.ModuleList([EncoderLayer(d_model,d_k,d_v,n_heads,d_ff) for _ in range(n_layers)])

    def forward(self, enc_inputs): # enc_inputs : [batch_size x source_len]
        enc_outputs = enc_inputs + self.pos_emb(torch.LongTensor([[1,2,3,4,0]]))
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs): # dec_inputs : [batch_size x target_len]
        dec_outputs = dec_inputs + self.pos_emb(torch.LongTensor([[5,1,2,3,4]]))
...
class Transformer(nn.Module):
    def __init__(self,d_model,d_k,d_v,n_heads,d_ff):
        ...
        self.encoder = Encoder(d_model,d_k,d_v,n_heads,d_ff)
        self.decoder = Decoder(d_model,d_k,d_v,n_heads,d_ff)
